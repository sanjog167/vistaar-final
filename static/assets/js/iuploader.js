/*! Image Uploader - v1.0.0 - 15/07/2019
  ...
  (function ($) {
    $.fn.imageUploader = function (options) {
      ...
      plugin.init = function () {
        ...
        plugin.each(function (i, wrapper) {
          $.each($(wrapper), function () {
            let newName = 1;
            let $container = createContainer(newName);
            newName++;
      ...
      let createContainer = function (i) {
        let $container = $("<div>", { class: "image-uploader" }),
          $input = $("<input>", {
            type: "file",
            id: plugin.settings.imagesInputName + "-" + random(),
            name: plugin.settings.imagesInputName + "dynamicname_" + i,
            multiple: "",
          }).appendTo($container),
          ...
          $i = $("<i>", { class: "fa fa-picture-o", text: "" }).appendTo(
            $textContainer
          ),
      ...
      let createImg = function (src, id) {
        let $container = $("<div>", { class: "uploaded-image" }),
          ...
          $i = $("<i>", { class: "fa fa-picture-o", text: "clear" }).appendTo(
            $button
          );
        if (plugin.settings.preloaded.length) {
          ...
          let $preloaded = $("<input>", {
            type: "hidden",
            name: plugin.settings.preloadedInputName + "sadas",
            value: id,
          }).appendTo($container);
        } else {
